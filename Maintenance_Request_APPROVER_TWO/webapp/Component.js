sap.ui.define([
	...
	"sap/ui/Device",
	"Maintenance_Request_APPROVER_TWO/Maintenance_Request_APPROVER_TWO/model/models"
], function (UIComponent, Device, models) {
	"use strict";

	return UIComponent.extend("Maintenance_Request_APPROVER_TWO.Maintenance_Request_APPROVER_TWO.Component", {
		...
		init: function () {
			...
			var contextModel = new sap.ui.model.json.JSONModel(
				"/Maintenance_Request_APPROVER_TWOMaintenance_Request_APPROVER_TWO/bpmworkflowruntime/v1/task-instances/" + taskId + "/context");
			contextModel.setDefaultBindingMode(sap.ui.model.BindingMode.TwoWay);
			this.setModel(contextModel);
			var taskModel = new sap.ui.model.json.JSONModel(
				"/Maintenance_Request_APPROVER_TWOMaintenance_Request_APPROVER_TWO/bpmworkflowruntime/v1/task-instances/" + taskId);
			...
			startupParameters.inboxAPI.addAction({
				action: "APPROVE",
				label: resourceBundle.getText("Interno")
			}, function (button) {
		...
		_completeTask: function (taskId, approvalStatus, refreshTask) {
			...
			context.pedro_initial_approval_status = approvalStatus;
			var data = {
				status: "COMPLETED",
				context: context
			};
			$.ajax({
				url: "/Maintenance_Request_APPROVER_TWOMaintenance_Request_APPROVER_TWO/bpmworkflowruntime/v1/task-instances/" + taskId,
				method: "PATCH",
		...
		_fetchToken: function () {
			var token;
			$.ajax({
				url: "/Maintenance_Request_APPROVER_TWOMaintenance_Request_APPROVER_TWO/bpmworkflowruntime/v1/xsrf-token",
				method: "GET",
				async: false,
				headers: {
					"X-CSRF-Token": "Fetch"
